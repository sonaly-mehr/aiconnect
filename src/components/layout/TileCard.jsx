  ...
  return (
    ...
    >
      <div class="flip">
        ...
        {service?.descrp && 
          <p className="font-lightl font-geometria text-[11px] text-left mt-3">{service?.descrp}</p>
        }
          {service?.list && (
            <ul className="flex flex-col gap-1 mt-3 text-left px-2">
              {service?.list?.map((item) => (
                <li className={`font-lightl list-disc  font-geometria ${path==="/water-solution" ? 'text-[11px]' : 'text-[13px]'} text-left`}>
                  {item}
                </li>
              ))}
            </ul>
